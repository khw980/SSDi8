...
from hooks import AccumBranchHook, BaseInputPackager, BaseOutputPackager
...
class LowRankBranch(nn.Module):
    def __init__(
        ...
        if rank == 0:
            ...
            self.a, self.b = nn.Linear(in_features, out_features, bias=False), nn.Identity()                                #L1,L2 생성준비
        else:
            self.a, self.b = nn.Linear(in_features, rank, bias=False), nn.Linear(rank, out_features, bias=False)
        self.reset_parameters(weight)                                                                                       #L1 L2 생성 완료
    ...
    def reset_parameters(self, weight: torch.Tensor | None = None) -> None:
        ...
        if self.rank < 0:
            ...
            u, s, vh = torch.linalg.svd(weight.double())                                                            #여기서 분해
    ...
    def get_effective_weight(self) -> torch.Tensor | None:                                                          #L1 x L2로 텐서 반환
        if self.rank == 0:
            return None
        elif self.rank < 0:
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor | None:
        if self.a is None:
            ...
            if input.ndim <= 3:
                return self.alpha * self.b(self.a(input))                                                           #입력차원 3이하면 바로 
            else:
                assert input.ndim == 4
