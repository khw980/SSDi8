...
class MaskInputHook(IOHook):
    def __init__(self, bad_cols):
        super().__init__(pre=True, post=False,
                         input_packager=SimpleInputPackager())
        self.bad_cols = bad_cols

    def pre_forward(self, module, input_args, input_kwargs):
        x_int8 = input_args[0]                     # [B, L, D_in] int8
        print("\n[MaskHook] before mask  col 3 ->",
              x_int8[0,0, self.bad_cols[0]].item())
        x_int8[..., self.bad_cols] = 0             # 열(col) 마스킹
        # repack
        print("[MaskHook] after mask   col 3 ->",
        x_int8[0,0, self.bad_cols[0]].item())
        new_args = (x_int8,) + input_args[1:]
        return new_args, input_kwargs              # overwrite args