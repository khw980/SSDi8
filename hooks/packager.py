...
# hooks/packager.py
from .packager import BaseInputPackager

class DequantInputPackager(BaseInputPackager):
    def unpack(self, module, input_args, input_kwargs):
        x_int8 = input_args[0]
        scale  = module.input_scale
        x_fp   = (x_int8.float() * scale).to(module.weight.dtype)
        return {"x": x_fp}

    def repack(self, new_tensors, module, input_args, input_kwargs, output=None):
        x_fp = new_tensors["x"]
        new_args = (x_fp,) + input_args[1:]
        return new_args, input_kwargs