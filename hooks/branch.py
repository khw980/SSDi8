...
class AccumBranchHook(IOHook):
    ...
    had_low: nn.Module | None   # 👈 추가
    def __init__(
        self,
        branch: nn.Module | None,
        had_low: nn.Module | None = None,  # 👈 명시적 had 모듈 받기
    ...
    def pre_forward(
        ...
        # self.tensor = self.had_low(self.tensor)                      # 👈 명시적 had 적용
        self.scale  = module.input_scale.clone()
        return None
    ...
    def post_forward(
        ...
        if self.branch is not None:
            # 2️⃣ de-quant
            scale=self.scale                      # 스칼라
            x_fp  = (self.tensor.float() * scale).to(self.branch.a.weight.dtype)
            y_low = self.branch(x_fp)
            
            
            #y_low = self.branch(self.tensor)
            output_tensor.add_(y_low)                       # in-place 누적
