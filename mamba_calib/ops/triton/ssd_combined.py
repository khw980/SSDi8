...
from mamba_calib.ops.triton.ssd_bmm import _bmm_chunk_fwd, _bmm_chunk_bwd
from mamba_calib.ops.triton.ssd_chunk_state import _chunk_cumsum_fwd, _chunk_cumsum_bwd
from mamba_calib.ops.triton.ssd_chunk_state import _chunk_state_fwd, _chunk_state_bwd_db
from mamba_calib.ops.triton.ssd_chunk_state import _chunk_state_bwd_ddAcs_stable
from mamba_calib.ops.triton.ssd_chunk_state import chunk_state, chunk_state_ref, my_chunk_state_ref
from mamba_calib.ops.triton.ssd_chunk_state import chunk_state_varlen
from mamba_calib.ops.triton.ssd_state_passing import _state_passing_fwd, _state_passing_bwd
from mamba_calib.ops.triton.ssd_state_passing import state_passing, state_passing_ref
from mamba_calib.ops.triton.ssd_chunk_scan import _chunk_scan_fwd, _chunk_scan_bwd_dz, _chunk_scan_bwd_dstates
from mamba_calib.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_dC, _chunk_scan_bwd_dcb
from mamba_calib.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_stable
from mamba_calib.ops.triton.ssd_chunk_scan import chunk_scan, chunk_scan_ref, my_chunk_scan_ref
from mamba_calib.ops.triton.ssd_chunk_scan import _chunk_scan_bwd_ddAcs_prev
from mamba_calib.ops.triton.layernorm_gated import rmsnorm_fn, _layer_norm_fwd, _layer_norm_bwd
from mamba_calib.ops.triton.k_activations import _swiglu_fwd, _swiglu_bwd
...
# class _CudaTimer:
#     """Context manager for accurate CUDA timing + per-block peak mem."""
#     def __init__(self, tag: str, device: torch.device | None):
#         self.tag = tag
#         self.device = device
#         self.cuda = (device is not None) and ("cuda" in str(device))
#         if self.cuda:
...
#             # reset peak so the block's peak is isolated
#             torch.cuda.reset_peak_memory_stats(self.device)
#             self.alloc0 = torch.cuda.memory_allocated(self.device)
#             self.resv0  = torch.cuda.memory_reserved(self.device)
...
#             resv1  = torch.cuda.memory_reserved(self.device)
#             peak   = torch.cuda.max_memory_allocated(self.device)
#             #print(f"[TIME][{self.tag}] {ms:.3f} ms | memΔ alloc={_fmt_bytes(alloc1 - self.alloc0)}, reserved={_fmt_bytes(resv1 - self.resv0)}, peak={_fmt_bytes(peak)}")
#         else:
#             #dt = (time.perf_counter() - self.t0) * 1000.0
#             #print(f"[TIME][{self.tag}] {dt:.3f} ms (CPU)")
# import nvtx
...
#     dev = x.device
#         #with _CudaTimer("chunk_cumsum_fwd", dev):
#     dA_cumsum, dt = _chunk_cumsum_fwd(dt, A, chunk_size, dt_bias=dt_bias, dt_softplus=dt_softplus, dt_limit=dt_limit)
#     #print("=== _mamba_chunk_scan_combined_fwd :: INTERMEDIATES/OUTPUT ===")
#     #_tensor_mem_report("dA_cumsum", dA_cumsum)
#     #_tensor_mem_report("dt (processed)", dt)
#     #with nvtx.annotate("chunk_state_fwd", color="green"):
#         #with _CudaTimer("chunk_state_fwd", dev):
#     states = _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=seq_idx, states_in_fp32=True)
#     #_tensor_mem_report("states_before_passing", states)
...
#    # with nvtx.annotate("state_passing_fwd", color="yellow"):
#         #with _CudaTimer("state_passing_fwd", dev):
...
#     #print("=== _mamba_chunk_scan_combined_fwd :: INTERMEDIATES/OUTPUT ===")
#     #_tensor_mem_report("states_after_passing", states)
#     #_tensor_mem_report("final_states", final_states)
#     # states_tmp0 = rearrange(_state_passing_fwd(rearrange(states_tmp0, "... p n -> ... (p n)"), dA_cumsum_tmp0[:, :, :, -1], chunk_size=chunk_size), "... (p n) -> ... p n", n=dstate)
#     # states_tmp1 = rearrange(_state_passing_fwd(rearrange(states_tmp1, "... p n -> ... (p n)"), dA_cumsum_tmp1[:, :, :, -1], chunk_size=chunk_size), "... (p n) -> ... p n", n=dstate)
#    # with nvtx.annotate("bmm_chunk_fwd", color="red"):
#         #with _CudaTimer("bmm_chunk_fwd", dev):
#     CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
#     #_tensor_mem_report("CB", CB)
#     #with nvtx.annotate("chunk_scan_fwd", color="purple"):
#         #with _CudaTimer("chunk_scan_fwd", dev):
#     out, out_x = _chunk_scan_fwd(CB, x, dt, dA_cumsum, C, states, D=D, z=z, seq_idx=seq_idx)
#     #_tensor_mem_report("out", out)
...
def _mamba_chunk_scan_combined_fwd(
    x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None,
    initial_states=None, seq_idx=None, cu_seqlens=None,
    dt_softplus=False, dt_limit=(0.0, float("inf"),),collect_obs=None
):
    batch, seqlen, nheads, headdim = x.shape
    _, _, ngroups, dstate = B.shape
    ...
    def _time_block(label: str, device=None, sync: bool=True, mem: bool=True, reset_peak: bool=False):
            ...
            try:
                ...
                if use_cuda:
                    ...
                    # except Exception:
                    #     pass
    dev = x.device if x.is_cuda else None
    dA_cumsum, dt = _chunk_cumsum_fwd(
                dt, A, chunk_size, dt_bias=dt_bias, dt_softplus=dt_softplus, dt_limit=dt_limit
            )

    states = _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=seq_idx, states_in_fp32=True,collect_obs=collect_obs)
    if collect_obs is not None:
        amax_state = states.abs().amax(dim=(0, -1, 1))  # (H,P)
        if (obs := collect_obs.get("state_cali", None)) is not None:
            # 기존 옵저버면 obs.update_amax(...) 하나 추가하세요
            obs.update_amax(amax_state)
    ...
    out, out_x = _chunk_scan_fwd(CB, x, dt, dA_cumsum, C, states, D=D, z=z, seq_idx=seq_idx,collect_obs=collect_obs)
    ...
    if cu_seqlens is None:
        ...
        varlen_states = chunk_state_varlen(
            B.squeeze(0), x.squeeze(0), dt.squeeze(0), dA_cumsum.squeeze(0),
            cu_seqlens, states.squeeze(0)
        )
        return out, out_x, dt, dA_cumsum, states, final_states, varlen_states
...
def _tensor_mem_report(name, t):
    ...
    try:
        ...
        storage_nbytes = t.storage().nbytes() if t.storage() is not None else 0
    elem = t.element_size()
    numel = t.numel()
    nbytes = elem * numel
    print(
        f"[{name}] shape={tuple(t.shape)} | dtype={t.dtype} | device={t.device} | "
        f"stride={t.stride()} | contiguous={t.is_contiguous()}"
    )
    print(
        f"        elements={numel:,} | elem_size={elem}B | "
        f"tensor_bytes={_fmt_bytes(nbytes)} | storage_bytes={_fmt_bytes(storage_nbytes)}"
    )

class MambaChunkScanCombinedFn(torch.autograd.Function):

    @staticmethod
    def forward(ctx, x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), return_final_states=False, return_varlen_states=False,collect_obs=None):
        # #############################################################################debuggin###########################################################
        ################################################################################################################################################
        ...
        if x.is_cuda:
            ...
            start.record()
        out, out_x, dt_out, dA_cumsum, states, final_states, *rest = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, cu_seqlens=cu_seqlens, dt_softplus=dt_softplus, dt_limit=dt_limit,collect_obs=collect_obs,)
...
def mamba_chunk_scan_combined(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), return_final_states=False, return_varlen_states=False,collect_obs=None):
    """
    Argument:
        x: (batch, seqlen, nheads, headdim)
    ...
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    return MambaChunkScanCombinedFn.apply(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus, dt_limit, return_final_states, return_varlen_states,collect_obs)
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    from mamba_calib.ops.selective_scan_interface import selective_scan_fn
