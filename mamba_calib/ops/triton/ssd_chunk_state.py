...
from mamba_calib.ops.triton.softplus import softplus
...
def _chunk_state_fwd_kernel(
    ...
    gid = pid_h // nheads_ngroups_ratio  # head→group
...
def _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=None, states=None, states_in_fp32=True,collect_obs: dict | None = None):
    ...
    dA_last  = dA_cumsum[..., -1:].contiguous()
    scale_lut = torch.exp(dA_last - dA_cumsum) * dt
    if seq_idx is not None:
        seg  = seq_idx.view(batch, nchunks, chunk_size)
        mask = seg.eq(seg[..., -1:].expand_as(seg)).unsqueeze(1)  # (B,1,C,K)
        scale_lut = scale_lut * mask
    scale_lut = scale_lut.to(torch.float16)


    S_total = nchunks * chunk_size
    if seqlen != S_total:
        x_pad = torch.zeros((batch, S_total, nheads, headdim), device=x.device, dtype=x.dtype)
        x_pad[:, :seqlen] = x
        x = x_pad
    if B.shape[1] != S_total:
        B_pad = torch.zeros((batch, S_total, ngroups, dstate), device=B.device, dtype=B.dtype)
        B_pad[:, :seqlen] = B
        B = B_pad

    seqlen_kernel = S_total

    X_bcKhd = x.view(batch, nchunks, chunk_size, nheads, headdim)     # (B,C,K,H,D)
    if collect_obs is not None:
        ori_amax_chp = X_bcKhd.abs().amax(dim=(0, 2, 1))  # (H,P)
        if (obs := collect_obs.get("ori_x_chp", None)) is not None:
            # 기존 옵저버면 obs.update_amax(...) 하나 추가하세요
            obs.update_amax(ori_amax_chp)
    scale_bckh1 = scale_lut.permute(0, 2, 3, 1).unsqueeze(-1)         # (B,C,K,H,1)
    X_scaled = (X_bcKhd * scale_bckh1).contiguous()
    if collect_obs is not None:
        amax_chp = X_scaled.abs().amax(dim=(0, 2, 1))  # (H,P)
        if (obs := collect_obs.get("x_chp", None)) is not None:
            # 기존 옵저버면 obs.update_amax(...) 하나 추가하세요
            obs.update_amax(amax_chp)
            b_bckgn = B.view(batch, nchunks, chunk_size, ngroups, dstate)
            amax_cgn = b_bckgn.abs().amax(dim=(0, 2 , 1))  # (G,N)
            if (obs := collect_obs.get("B_cgn", None)) is not None:
                obs.update_amax(amax_cgn)
    X_scaled = X_scaled.view(batch, S_total, nheads, headdim)
    if X_scaled.stride(-1) != 1:
        X_scaled = X_scaled.contiguous()
    if B.stride(-1) != 1:
        B = B.contiguous()
    ...
    with torch.cuda.device(x.device.index):
        _chunk_state_fwd_kernel[grid](
            ...
            # strides: X (B,S,H,D)
            X_scaled.stride(0), X_scaled.stride(1), X_scaled.stride(2), X_scaled.stride(3),
            # strides: B (B,S,G,N)
            B.stride(0), B.stride(1), B.stride(2), B.stride(3),
            # strides: states (B,C,H,D,N)
