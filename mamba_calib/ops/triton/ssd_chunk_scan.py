...
def _chunk_scan_fwd(cb, x, dt, dA_cumsum, C, states, D=None, z=None, seq_idx=None,collect_obs: dict | None = None):
    ...
    if collect_obs is not None:
            amax_C = C.abs().amax(dim=(0, 1 ,3))  # (,G)
            if (obs := collect_obs.get("C_cali", None)) is not None:
                obs.update_amax(amax_C)
                amax_cb = cb.abs().amax(dim=(0, 3 , 1, 4))  # (G,)
                if (obs := collect_obs.get("cb_cali", None)) is not None:
                    obs.update_amax(amax_cb)
