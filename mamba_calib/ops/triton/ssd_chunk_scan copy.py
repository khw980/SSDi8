...
def _chunk_scan_fwd_kernel(
    ...
    dt_ptr, dA_cumsum_ptr,
    C_ptr, prev_states_ptr, D_ptr,
    prev_row_scale_ptr,  # (B,H,C,K) row-scale for prev path (exp(dA_m) * gate)
    # Sizes
    ...
    stride_D_head,
    stride_prevscale_batch, stride_prevscale_chunk, stride_prevscale_head, stride_prevscale_csize,
    # Meta
    ...
    prev_row_scale_ptr += pid_b * stride_prevscale_batch + pid_c * stride_prevscale_chunk + pid_h * stride_prevscale_head
    ...
    # ---------------- prev: acc_prev = (C @ prev_states) * prev_row_scale ----------------
    ...
    if BLOCK_SIZE_DSTATE <= 128:
        ...
        prev_states = prev_states.to(C.dtype)
    ...
    prev_scale_m = tl.load(prev_row_scale_ptr + offs_m * stride_prevscale_csize,
                           mask=offs_m < chunk_size_limit, other=0.0).to(tl.float32)
    acc *= prev_scale_m[:, None]

    # ---------------- conv: ORIGINAL STYLE (cb @ x with in-kernel dA/dt) ----------------
    ...
    acc_conv = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.float32)
    K_MAX = chunk_size_limit if not IS_CAUSAL else tl.minimum((pid_m + 1) * BLOCK_SIZE_M, chunk_size_limit)
    for k in range(0, K_MAX, BLOCK_SIZE_K):
        # column dA, dt
        dA_cs_k = tl.load(dA_cumsum_ptrs, mask=offs_k < chunk_size - k, other=0.0).to(tl.float32)
        dt_k    = tl.load(dt_ptrs,        mask=offs_k < chunk_size - k, other=0.0).to(tl.float32)

        cb = tl.load(cb_ptrs, mask=(offs_m[:, None] < chunk_size_limit) & (offs_k[None, :] < chunk_size - k), other=0.0)
        if IS_CAUSAL:
            mask = offs_m[:, None] >= k + offs_k[None, :]
            cb = tl.where(mask, cb, 0.0)

        # scale: exp(dA_m - dA_k) * dt_k
        scale = tl.exp(dA_cs_m[:, None] - dA_cs_k[None, :]) * dt_k[None, :]
        cb = (cb * scale).to(x_ptr.dtype.element_ty)

        xs = tl.load(x_ptrs, mask=(offs_k[:, None] < chunk_size_limit - k) & (offs_n[None, :] < hdim), other=0.0)
        acc_conv += tl.dot(cb, xs)
    ...
    if HAS_D:
        ...
        x_res = tl.load(x_ptr + (offs_m[:, None] * stride_x_seqlen + offs_n[None, :] * stride_x_hdim),
...
def _chunk_scan_fwd(cb, x, dt, dA_cumsum, C, states, D=None, z=None, seq_idx=None, collect_obs: dict | None = None):
    """
    Hybrid: prev-state(C@states)은 바깥에서 row-scale(gate*exp(dA_m))을 만들어 커널에서 곱함.
            conv(cb@x)은 원본 방식(커널 내부에서 exp(dA_m - dA_k)*dt_k 적용)으로 복귀.
    I/O shapes는 기존과 동일: (B, seqlen, H, D)
    """
    ...
    # -------- prev row-scale (exp(dA_m) with gate from seq_idx) --------
    out_row_exp = torch.exp(dA_cumsum.to(torch.float32))  # (B,H,C,K)
    if seq_idx is not None:
        S_total = nchunks * chunk_size
        seg_full = torch.full((batch, S_total), -1, device=x.device, dtype=seq_idx.dtype)
        seg_full[:, :seqlen] = seq_idx
        seg_full = seg_full.view(batch, nchunks, chunk_size)
        prev_id = torch.full((batch, nchunks), -1, device=x.device, dtype=seq_idx.dtype)
        prev_id[:, 1:] = seg_full[:, :-1, -1]
        gate = seg_full.eq(prev_id.unsqueeze(-1))  # (B,C,K), True=연속
        prev_row_scale = (out_row_exp * gate.unsqueeze(1)).to(torch.float32)  # (B,H,C,K)
    else:
        prev_row_scale = out_row_exp

    # -------- observers as requested --------
    if collect_obs is not None:
        # 1) x: (H, P)
        amax_hp = x.abs().amax(dim=(0, 1))
        if (obs := collect_obs.get("x_hp", None)) is not None:
            obs.update_amax(amax_hp)
        # 2) C: (G, K)
        S_total = nchunks * chunk_size
        if C.shape[1] != S_total:
            C_pad = torch.zeros((batch, S_total, ngroups, dstate), device=C.device, dtype=C.dtype)
            C_pad[:, :C.shape[1]] = C
            C_obs = C_pad
        else:
            C_obs = C
        C_bckgn = C_obs.view(batch, nchunks, chunk_size, ngroups, dstate)
        amax_gk = C_bckgn.abs().amax(dim=(0, 1, 4)).permute(1, 0).contiguous()
        if (obs := collect_obs.get("C_gk", None)) is not None:
            obs.update_amax(amax_gk)
        # 3) prev_state: (H, P)
        amax_prev_hp = states.abs().amax(dim=(0, 1, 4))
        if (obs := collect_obs.get("prev_hp", None)) is not None:
            obs.update_amax(amax_prev_hp)

    # -------- outputs --------
    out   = torch.empty(batch, seqlen, nheads, headdim, device=x.device, dtype=x.dtype)
    out_x = torch.empty_like(out) if z is not None else None
    ...
    with torch.cuda.device(x.device.index):
        _chunk_scan_fwd_kernel[grid](
            ...
            prev_row_scale.stride(0), prev_row_scale.stride(2), prev_row_scale.stride(1), prev_row_scale.stride(3),
            # meta (키워드!)
            IS_CAUSAL=True,
            HAS_D=(D is not None),
